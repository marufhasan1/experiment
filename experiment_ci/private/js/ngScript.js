...
//var url = 'http://' + window.location.hostname + '/pcpl/ajax/';
//var siteurl = 'http://' + window.location.hostname + '/pcpl/';
...
app.controller("showcategoryCtrl", function($scope, $http, $log){
	...
	$scope.$watch("showroom_id", function(){
		var where = {
			table:'category',
			//cond:{showroom_id:$scope.showroom_id}
		};
...
//Add Sub Category
app.controller("addSubcategoryCtrl",function($scope,$http,$log){
	$scope.allcategory = [];
	$scope.setAllcategory = function(){
		var condition = {
			table: 'category',
...
//showroom all Category
app.controller("showroomCtrl",function($scope,$http,$log){
	$scope.reverse 		= false;
	$scope.perPage 		= "30";
	$scope.showrooms 	= [];
	$scope.resultset 	= [];
	...
	$http({
		...
		angular.forEach(response,function(values, index){
			// values['showroom_id'] = parseInt(values.showroom_id);

			var where = {
				table: 'showroom',
				cond: {showroom_id : values.showroom_id}
			};
			...
			$http({
				...
				angular.forEach(alltype, function(value, index){
					values.types.push(value.type);
				});
			});

			values['sl'] = index + 1;

			$scope.showrooms.push(values);
			$scope.resultset.push(values);
...
app.controller("DistributeCtrl", function($scope, $http){



	$scope.getStockFn = function(){
		//console.log($scope.doRecord);
		//console.log($scope.doRecord.do_no);
		var where = {
			table: 'stock',
			cond: {
				product_code: $scope.product,
				do_no: $scope.doRecord.do_no,
				showroom_id: 'godown',
				type: 'do'
			}
		}
		...
		$http({
			...
			if (response.length > 0) {
				...
				$scope.info.purchase_price = parseFloat(response[0].purchase_price);
				$scope.info.remainingDo = response[0].do_in - response[0].do_out;
	...
	$scope.getTotalAmount = function(){
		var total = 0.00;
		total = $scope.info.purchase_price * $scope.distributeQuantity;
		...
		if(typeof $scope.product !== 'undefined'){
			...
			$http({
				...
				var item = {
					...
					sellPrice: parseFloat(response[0].sale_price),
...
// add purchase entry
app.controller('doPurchaseEntry', function($scope, $http){
	$scope.active = true;
	$scope.cart = [];
	$scope.amount = {
		total: 0,
...
// DO Edit Purchase Entry
app.controller('DoEditPurchaseEntry', function($scope, $http) {
	$scope.partyInfo = {
		partyCode: '',
		previousBalance: 0.00,
		currentBalance: 0.00,
	...
	$scope.$watch('vno', function(voucherNo) {
		...
		$http({
			...
			if(response.length > 0) {
				...
				$scope.amount.partyCode = response[0].party_code;
				$scope.amount.sapType = response[0].sap_type;
				$scope.amount.doType = response[0].do_type;
...
app.controller('SaleEntryCtrl', function($scope, $http) {
  ...
  $scope.isDisabled = false;
	...
	/*$scope.getGrandTotalFn = function(){
		var grand_total = 0.0;
		grand_total = $scope.amount.total - $scope.amount.discount;
		return $scope.amount.grandTotal = grand_total.toFixed(2);
	}*/

	$scope.getGrandTotalFn = function(){
		var grand_total = 0.0;
		grand_total = (parseFloat($scope.amount.total) - parseFloat($scope.amount.discount) - parseFloat($scope.amount.truck_rent)) + parseFloat($scope.amount.labour);
		return $scope.amount.grandTotal = grand_total.toFixed(2);
	}
		...
		$http({
			...
			if(response.length > 0) {
				...
				$http({
					...
					if(response.length > 0){
						angular.forEach(response, function(row, index){
							if(row.meta_key == "truck_quantity"){$scope.truck.quantity = parseInt(row.meta_value);}
							if(row.meta_key == "truck_amount"){$scope.truck.amount = parseFloat(row.meta_value);}
							if(row.meta_key == "commission_quantity"){$scope.commission.quantity = parseInt(row.meta_value);}
							if(row.meta_key == "commission_amount"){$scope.commission.amount = parseFloat(row.meta_value);}
							if(row.meta_key == "labour_cost"){
								$scope.amount.labour = parseFloat(row.meta_value);
								$scope.oldLabourCost = parseFloat(row.meta_value);
							}
							if(row.meta_key == "truck_total"){$scope.oldTruckRent = parseFloat(row.meta_value);}
...
app.controller("CustomSMSCtrl", ["$scope", "$log", function($scope, $log){
	...
	$scope.$watch(function(){
		var charLength = $scope.msgContant.length + 30,
			messLen = 0;
...
app.controller('CompanyTransactionCtrl', ['$scope', '$http', function($scope, $http) {
	...
	$scope.getCompanyInfo = function() {
		var condition = {
			table: 'partybalance',
			cond: {'code' : $scope.code} // cond: {code: $scope.client.code}
		};
		...
		$http({
			...
			if(response.length > 0){
				...
				angular.forEach(response, function(values){
					values.status = (parseFloat(values.balance) <= 0 ) ? "receivable" :"payable";
					values.amount = Math.abs(parseFloat(values.balance));
